...
/**
 * Auto-commit state file changes to main repository
 * Called after saveState() to ensure changes are immediately committed
 *
 * Pre-commit hook explicitly allows .stream-state.json commits from main directory
 * (see .git/hooks/pre-commit line 51)
 */
async function commitStateFile(): Promise<void> {
  try {
    const git = simpleGit(config.PROJECT_ROOT);
    const statePathRelative = config.STREAM_STATE_PATH; // Relative to PROJECT_ROOT

    // Check if file is modified or untracked
    const status = await git.status();
    const isChanged =
      status.modified.includes(statePathRelative) ||
      status.not_added.includes(statePathRelative) ||
      status.created.includes(statePathRelative);
    ...
    // Stage and commit
    await git.add(statePathRelative);
    await git.commit('chore: Update stream state counter\n\nAuto-committed by mcp-stream-workflow state-manager');

    console.error('[state-manager] Auto-committed state file changes');
  } catch (error) {
    // Non-fatal: Log warning but don't throw
    // State file is saved even if commit fails
    console.error('[state-manager] Warning: Could not auto-commit state file:', error);
  }
}
 ...
 * Auto-commits changes to git after successful save
 */
